...
public class Comment extends WeiboResponse implements java.io.Serializable {
     ...
     */
    public static List<Comment> getComments(Response res) throws weibo4j.model.WeiboException, WeiboException {
        try {
            JSONArray comments = res.asJSONArray();
            int size = comments.length();
            List<Comment> res1 = new ArrayList<Comment>(size);
            for (int i = 0; i < size; i++) {
                res1.add(new Comment(comments.getJSONObject(i)));
            }
            return res1;
    ...
    public static CommentWapper constructWapperComments(Response res) throws WeiboException, weibo4j.model.WeiboException {
        ...
        try {
            ...
            ret = new CommentWapper(comment, previousCursor, nextCursor, totalNumber, hasvisible);
            return ret;
        } catch (JSONException jsone) {
            throw new WeiboException(jsone);
        } catch (Exception e) {
